...
function getNames(arr) {
  //original version
  const presNamesArr = arr.map((onePres) => {
    return onePres.name;
  });
  console.log({ arr, presNamesArr });
  return presNamesArr;

  //oneliner version
  // return arr.map((onePres) => onePres.name);
}

// Iteration 2 | Democratic Presidents - `filter()`
function getDemocraticPresidents(presidentsArr) {
  //original version
  const democraticPresArr = presidentsArr.filter((currentPres) => {
...
function countYearsInOffice(presidentsArr) {
  const totalYears = presidentsArr.reduce((acc, currentPres) => {
    if (currentPres.leftOffice) {
      const yearsInOffice = currentPres.leftOffice - currentPres.tookOffice;
